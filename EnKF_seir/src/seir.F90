...
!  Observations
   ...
   dobs(1) =1    ! total deaths at day 31/3
   dobs(2) =12   ! total hospitalized at day 31/3
   R(1,1)=0.1; R(1,2)=0.0
   R(2,1)=0.0; R(2,2)=1.0
...
! MODEL PARAMETERS (Set first guess (ensemble mean) of parameters (decleared in mod_parameters.F90) and their stddev 
   ...
   print *,'N=',N
   I0                = 1                        ; parstd(3)=6.0    ! 3  Initial infectious (19 cases 1st march)
   R0                = 2.7                          ; parstd(4)=0.5    ! 4  Basic Reproduction Number
   ...
   Rt                = 0.95                          ; parstd(12)=0.020  ! 13 Basic Reproduction Number during intervention
   InterventionTime  = 26                         ; parstd(13)=0.0   ! 14 Interventions start here (15th march)

   duration= 60 ! 45.0                             ! Duration of measures
...
!         print '(a,4f12.2)','SUM=',sum(y(0:neq-1))
      ...
      enddo
   enddo
   !Modified by Santiago Lopez-Restrepo
   call python(ens,enspar,nt,nrens,neq,nrpar,0,dobs)
   if (.not.lenkf) stop
...
! Posterior ensemble prediction
   ...
   !Modified by Santiago Lopez-Restrepo
   call python(ens,enspar,nt,nrens,neq,nrpar,1,dobs)
    CALL EXECUTE_COMMAND_LINE ('python Graphs_SEIR.py')
end program
